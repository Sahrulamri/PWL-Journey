    ...
    <div class="cartArea">
        ...
        if (!empty($_SESSION['cart'])) {
            echo "Jml data :" . sizeof($_SESSION['cart']['arrCart']) .
                "&nbsp&nbsp<a class='btnCart' href=cart-remove.php>Kosongkan
Cart</a>";
 ...
 <tbody>";
            ...
            for ($i = 0; $i < $max; $i++) {
                ...
                foreach ($_SESSION['cart']['arrCart'][$i] as $key => $val) {
                    ...
                    $tothrg = $hrg * $jml;
                    if ($key == "jml" || $key == "hrg")
                        $teks .= "<td>" . number_format($val, 0) .
                            "</td>";
                    else
                        $teks .= "<td>" . $val . "</td>";
                }
                $sum += $tothrg;
                $teks .= "<td class='kolom-kanan'>" .
                    number_format($tothrg, 0) . "</td>";
                $teks .= "<td> <a href=cart-item-remove.php?idx=" . $i . "
class='btnCart'>remove</a></td>";
                $teks .= "</tr>";
            }
            $teks .= "<tr><td colspan=6 style='text-align:right;'>Total harga: " .
                number_format($sum, 0) . "</td><tr>";
            $teks .= "</tbody></table>";
            echo $teks;
        } else
            echo "cart kosong";
